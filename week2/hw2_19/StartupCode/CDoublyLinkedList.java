public class CDoublyLinkedList {
	...
	public boolean removeStartingAtBack(Object dataToRemove) {
		...
		while(last != this.head) {
			if(dataToRemove == null ? last.data == null : dataToRemove.equals(last.data)) {
	...
	public boolean retainAll(CDoublyLinkedList other) throws NullPointerException {
		if(other == null) {
			throw new NullPointerException("Other is not found");
		}
		Node cur = this.head.next;
		boolean flag;
		while(cur != this.head) {
	...
	// Contains helper method checks if the passed
	// parameter is null and returns true.
	// Also, if other equals cur's data
	// return true.
